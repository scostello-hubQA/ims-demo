...
create table if not exists ims.orderline(orderline_id int primary key auto_increment, item_id int, order_id int, quantity int, total_price double (7,2), foreign key(item_id) references ims.items(item_id), foreign key(order_id) references ims.orders(order_id));
